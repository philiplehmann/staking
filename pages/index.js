...
const Home = memo(() => {
  ...
  const groupedProgression = useMemo(() => {
    ...
    for (let i = 0; i < rounds; i++) {
      ...
      progression.push({ capital: currentCapital, days: currentDays, date: new Date(now + currentDays * 24 * 3600 * 1000) })
    }
  ...
  return (
    <div>
      ...
      <main>
        <div>rate <input name="rate" value={rate} onChange={setRateHandler} /></div>
        <div>capital <input name="capital" value={capital} onChange={setCapitalHandler} /></div>
        <div>percent <input name="percent" value={percent} onChange={setPercentHandler} /></div>
        <div>payouts <input name="payouts" value={payouts} onChange={setPayoutsHandler} /></div>
        <div>years <input name="years" value={years} onChange={setYearsHandler} /></div>
